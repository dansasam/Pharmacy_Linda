...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($drugName === '' || $manufacturer === '' || $recordDate === '' || $invoiceNo === '' || $currentInventory < 0) {
        ...
        $stmt = $pdo->prepare('INSERT INTO product_inventory (drug_name, manufacturer, record_date, invoice_no, current_inventory, initial_comments) VALUES (?, ?, ?, ?, ?, ?)');
        $stmt->execute([$drugName, $manufacturer, $recordDate, $invoiceNo, $currentInventory, $initialComments]);
        header('Location: process7_9_inventory.php?message=' . urlencode('Inventory entry added.'));
        exit;
    }
}
?>
        ...
        <main class="main-content">
            ...
            <section class="section-card">
                ...
                <form method="post" class="compact-form">
                    ...
                    <input type="text" name="drug_name" value="<?php echo sanitize_text($_POST['drug_name'] ?? ''); ?>" required />

                    <label>Manufacturer</label>
                    <input type="text" name="manufacturer" value="<?php echo sanitize_text($_POST['manufacturer'] ?? ''); ?>" required />

                    <label>Date</label>
                    <input type="date" name="record_date" value="<?php echo sanitize_text($_POST['record_date'] ?? ''); ?>" required />

                    <label>Invoice Number</label>
                    <input type="text" name="invoice_no" value="<?php echo sanitize_text($_POST['invoice_no'] ?? ''); ?>" required />

                    <label>Current Inventory</label>
                    <input type="number" min="0" name="current_inventory" value="<?php echo sanitize_text($_POST['current_inventory'] ?? ''); ?>" required />

                    <label>Initial Comments</label>
                    <textarea name="initial_comments" rows="3"><?php echo sanitize_text($_POST['initial_comments'] ?? ''); ?></textarea>
